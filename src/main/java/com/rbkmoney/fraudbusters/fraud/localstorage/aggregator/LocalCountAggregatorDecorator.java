...
public class LocalCountAggregatorDecorator implements CountPaymentAggregator<PaymentModel, PaymentCheckedField> {
    ...
    @Override
    public Integer count(PaymentCheckedField checkedField, PaymentModel paymentModel, TimeWindow timeWindow,
    ...
    @Override
    public Integer countSuccess(PaymentCheckedField checkedField, PaymentModel paymentModel, TimeWindow timeWindow,
                                List<PaymentCheckedField> list) {
        Integer countError = countAggregator.countSuccess(checkedField, paymentModel, timeWindow, list);
        Integer resultCount = getCount(checkedField, paymentModel, timeWindow, list,
                localStorageRepository::countOperationSuccessWithGroupBy) + countError;
    ...
    public Integer countError(PaymentCheckedField checkedField, PaymentModel paymentModel, TimeWindow timeWindow,
        ...
        try {
            ...
            Instant instantTo = Instant.ofEpochMilli(TimestampUtil.generateTimestampMinusMinutesMillis(now,
                    timeWindow.getEndWindowTime()));
            Integer localCount = localStorageRepository.countOperationErrorWithGroupBy(checkedField.name(),
                    resolve.getValue(),
                    instantFrom.getEpochSecond(),
                    instantTo.getEpochSecond(),
                    eventFields, errorCode);
            int result = localCount + countError;
            log.debug("LocalStorageCountAggregatorImpl field: {} value: {}  countError: {}", resolve.getName(),
    ...
    @Override
    public Integer countChargeback(PaymentCheckedField checkedField, PaymentModel model, TimeWindow timeWindow,
                                   List<PaymentCheckedField> fields) {
        return countAggregator.countChargeback(checkedField, model, timeWindow, fields);
    }

    @Override
    public Integer countRefund(PaymentCheckedField checkedField, PaymentModel model, TimeWindow timeWindow,
                               List<PaymentCheckedField> fields) {
    ...
    private Integer getCount(
        ...
        try {
            ...
            log.debug("LocalStorageCountAggregatorImpl field: {} value: {}  count: {}", resolve.getName(),
                    resolve.getValue(), count);
            return count;
        } catch (Exception e) {
