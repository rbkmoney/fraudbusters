...
public class ChargebackEventListener {
    ...
    @KafkaListener(topics = "${kafka.topic.event.sink.chargeback}", containerFactory =
            "kafkaChargebackResultListenerContainerFactory")
    public void listen(List<Chargeback> chargeback, @Header(KafkaHeaders.RECEIVED_PARTITION_ID) Integer partition,
        ...
        try {
            log.info("ChargebackEventListener listen result size: {} partition: {} offset: {}", chargeback.size(),
                    partition, offset);
