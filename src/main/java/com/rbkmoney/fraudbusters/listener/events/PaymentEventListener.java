...
public class PaymentEventListener {
    ...
    @KafkaListener(topics = "${kafka.topic.event.sink.payment}", containerFactory =
            "kafkaPaymentResultListenerContainerFactory")
    public void listen(List<Payment> payments, @Header(KafkaHeaders.RECEIVED_PARTITION_ID) Integer partition,
        ...
        try {
            log.info("PaymentEventListener listen result size: {} partition: {} offset: {}", payments.size(),
                    partition, offset);
